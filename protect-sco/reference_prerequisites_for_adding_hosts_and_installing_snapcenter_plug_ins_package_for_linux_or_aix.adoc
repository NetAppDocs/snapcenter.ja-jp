...
---
= ホストを追加し、Linux または AIX 向け SnapCenter プラグインパッケージをインストールするための前提条件
:allow-uri-read: 
:icons: font
:imagesdir: ../media/
...
* Java 17（SnapCenter 6.2.1 以降の場合）または Java 11（SnapCenter 6.2 以前の場合）を Linux または AIX ホストにインストールしている必要があります。
+
** Linuxでは、Oracle、OpenJDK、またはSAP JDKのJavaがサポートされています。
** IBM Java for AIX。 https://developer.ibm.com/languages/java/semeru-runtimes/downloads/?version=17&os=AIX["IBM Semeruランタイムのダウンロード"^]からダウンロードできます。
+

NOTE: Linux ホストに Java 17 の認定版のみ（SnapCenter 6.2.1 以降の場合）または Java 11（SnapCenter 6.2 以前の場合）がインストールされていることを確認してください。
...
必要なソフトウェアパッケージ
 a| 
* Java 17 （Oracle Java、OpenJDK、またはSAP JDK）（SnapCenter 6.2.1以降）またはJava 11（SnapCenter 6.2以前）



NOTE: Linux ホストに SnapCenter 6.2.1 以降用の Java 17 の認定版のみ、または SnapCenter 6.2 以前用の Java 11 の認定版のみがインストールされていることを確認してください。
...
必要なソフトウェアパッケージ
 a| 
* IBM Java 17（SnapCenter 6.2.1以降）またはIBM Java 11（SnapCenter 6.2以前）
...
CAUTION: ホストの導入とプラグインのインストール後にクレデンシャルを作成することもできますが、* ベストプラクティス * は、SVM を追加した後、ホストを導入してプラグインをインストールする前にクレデンシャルを作成することです。
...
NOTE: 認証情報を作成する際に誤った情報を入力した場合、エラーメッセージが表示されます。*Cancel*を選択してから、もう一度お試しください。

.手順
. 左側のナビゲーションペインで、*[リソース]*を選択し、リストから適切なプラグインを選択します。
. [ リソース ] ページで、 [ * 表示 ] リストから [ * データベース * ] を選択します。
. image:../media/filter_icon.gif["フィルタアイコン"]を選択し、次にホスト名とデータベースの種類を選択してリソースを絞り込みます。
+
次に、image:../media/filter_icon.gif["フィルタアイコン"]を選択してフィルターペインを閉じることができます。

. データベースを選択し、次に*データベース設定*＞*データベースの構成*を選択します。
...
+
詳細については、link:../protect-sco/task_define_a_backup_strategy_for_oracle_databases.html#preferred-nodes-in-rac-setup["RACセツトアツフノユウセンノオト"^]を参照してください。
