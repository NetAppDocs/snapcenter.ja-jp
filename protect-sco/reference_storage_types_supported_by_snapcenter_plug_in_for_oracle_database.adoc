...
---
= SnapCenter Plug-in for Oracle Databaseでサポートされるストレージ タイプ
:allow-uri-read: 
:icons: font
:imagesdir: ../media/
...
その他のハイパーバイザー
 a| 
KVM、Xen、Hyper-V、ハイパースケーラーハイパーバイザー上で動作するLinuxは、ゲストOSが以下の方法を使用して必要なONTAPストレージリソースに直接TCP/IPアクセスできる場合にサポートされます：
...
NOTE: VMware以外では、データストアおよび生デバイスのプレゼンテーションはサポートされていません。
