...
* Windows プラグイン ホストに PowerShell 7.4.13 がインストールされていることを確認してください。
* WindowsおよびLinuxプラグインホストにJava 17（Oracle JavaまたはOpenJDK）がインストールされていることを確認してください（SnapCenter 6.2のみの場合はJava 11）。
