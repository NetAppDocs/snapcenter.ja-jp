...
* ASP.NET Core Runtime 8.0.24（およびそれ以降のすべての 8.0.x パッチ）Hosting Bundle とPowerShell 7.4.13 以降が Windows プラグインホストにインストールされていることを確認してください。
* SnapCenter 6.2.1以降ではJava 17（Oracle JavaまたはOpenJDK）、SnapCenter 6.2以前ではJava 11が、WindowsおよびLinuxプラグインホストにインストールされていることを確認してください。
