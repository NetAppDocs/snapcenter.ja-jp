...
---
= ホストを追加してMySQL向けSnapCenterプラグインをインストールするための前提条件
:allow-uri-read: 
:icons: font
:imagesdir: ../media/
...
* ホストに Java 17 （Oracle Java、OpenJDK、または SAP JDK）（SnapCenter 6.2.1 以降の場合）または Java 11 （SnapCenter 6.2 以前の場合）をインストールしておく必要があります。
+
...
* MySQL 8.0より前のバージョンをご利用の場合は、MySQLXプラグインを手動でインストールして有効化する必要があります。MySQL 8.0以降では、MySQLXプラグインはデフォルトでインストールされ、有効化されています。
...
* Windowsホストに、SnapCenter 6.2.1以降の場合はJava 17（Oracle Java、OpenJDK、またはSAP JDK）を、SnapCenter 6.2以前の場合はJava 11をインストールしておく必要があります。
+
link:https://www.oracle.com/java/technologies/downloads/#java17["すべてのオペレーティング システム用の Java をダウンロード"]
