...
---
= Microsoft Exchange Server 用 SnapCenter プラグインをインストールするための前提条件
:allow-uri-read: 
:icons: font
:imagesdir: ../media/
...
* スタンドアロン構成およびデータベース可用性グループ構成には、Microsoft Exchange Server を使用する必要があります。サポートされている Microsoft Exchange Server のバージョンについては、link:https://imt.netapp.com/imt/#search["Interoperability Matrix Tool（IMT）"^]を参照してください。
...
* ファイアウォールで以下のポートがブロックされていないことを確認してください。ブロックされている場合、ホストの追加操作は失敗します。この問題を解決するには、動的ポート範囲を設定する必要があります。詳細については、 https://docs.microsoft.com/en-us/troubleshoot/windows-server/networking/configure-rpc-dynamic-port-allocation-with-firewalls["Microsoftのドキュメント"^]を参照してください。サポートされているバージョンのMicrosoft Exchange Serverについては、link:https://imt.netapp.com/imt/#search["Interoperability Matrix Tool（IMT）"^]を参照してください。
+
