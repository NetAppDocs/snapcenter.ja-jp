...
リソース名
 a| 
リソース名を入力します。このオプションは、リソースタイプとして非データボリュームを選択した場合にのみ使用できます。
...
. 概要を確認し、 [ 完了 ] をクリックします。
+
データベースは、 SID 、通信ホスト、関連するリソースグループとポリシー、全体的なステータスなどの情報とともに表示されます。



リソースへのアクセスをユーザに許可する場合は、ユーザにリソースを割り当てる必要があります。これにより、ユーザは、自身に割り当てられたアセットに対して権限のある処理を実行できます。

リンク : https://docs.netapp.com/us-en/snapcenter/install/task_add_a_user_or_group_and_assign_role_and_assets.html[Add ユーザーまたはグループとロールおよびアセットの割り当て ]

データベースの追加が完了したら、 SAP HANA データベースの詳細を変更できます。

SAP HANA リソースにバックアップが関連付けられている場合、次の項目は変更できません。

* マルチテナントデータベースコンテナ（ MDC ）： SID または HDBSQL クライアントホスト
* Single Container ： SID または HDBSQL クライアントホスト
