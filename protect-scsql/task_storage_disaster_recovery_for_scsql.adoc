...
[role="lead"]
SnapCenter Plug-in for SQL Server ストレージをリカバリするには、グローバル設定 * ページでストレージの DR モードを有効にします。
...
* DR モードで作成された新しいバックアップは、「 * Topology * 」ページの SnapMirror または SnapVault （セカンダリ）の下に表示されます。
+
