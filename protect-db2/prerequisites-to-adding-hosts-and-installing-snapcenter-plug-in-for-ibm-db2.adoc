...
---
= ホストを追加して IBM Db2 用 SnapCenter プラグインをインストールするための前提条件
:allow-uri-read: 
:icons: font
:imagesdir: ../media/
...
* rootユーザまたはroot以外のユーザに対してパスワードベースのSSH接続を有効にしておく必要があります。
* Linux または AIX ホストに、SnapCenter 6.2.1 以降の場合は Java 17 （Oracle Java、OpenJDK、または SAP JDK）を、SnapCenter 6.2 以前の場合は Java 11 をインストールしておく必要があります。
+
link:https://www.oracle.com/java/technologies/downloads/#java17["Linux 用 Java をダウンロード"^]

+
link:https://developer.ibm.com/languages/java/semeru-runtimes/downloads/?license=IBM["AIX 用 Java をダウンロード"^]
